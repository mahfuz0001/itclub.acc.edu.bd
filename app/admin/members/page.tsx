...
// but in a real-world scenario, you would uncomment the import from "@/types/admin.ts".
// The 'instagram' field is still excluded as per previous instructions.
export interface Member {
  ...
  createdAt: string;
  year: string;
  status: "pending" | "approved" | "rejected"; // Limited status types
  rollNumber?: string;
  // Contact Information
  phone?: string;
  address?: string;
...
export default function MembersPage() {
  ...
  const analytics = useMemo(() => {
    // ⚠️ FIX: Changed "active" to "approved" to match the Member status type
    const activeMembers = members.filter(
      (m) => m.status === "approved"
    ).length;
    
    // Inactive members are those not approved (pending or rejected)
  ...
  const fetchMembers = async () => {
    ...
    try {
      ...
      if (user.role === "panel" && user.assignedYear) {
        ...
        membersQuery = query(
          collection(db, "applications"),
          // Fetch all members regardless of status for the admin view
          // The status filter will handle the display subset
        );
      }
  ...
  // --- EXPORT FUNCTIONS (UNCHANGED logic based on previous request) ---
  ...
  const getExportData = (member: Member, type: ExportType) => {
    ...
    // Use member.website for the "Insta Profile" column in the export
    const genericSocialLink = member.website || "N/A"; 
    ...
    switch (type) {
      case "basic_id_social":
        return {
          ...
          "Other Social Profile": genericSocialLink, 
        };
  ...
  const exportSelectedFields = (type: ExportType) => {
    ...
    try {
      ...
      if (type === "all_detailed") {
          ...
          // Basic Social ID Export
          headers = [
            "Name", "Student ID", "Facebook Profile", "Other Social Profile" 
          ];
      } else {
  ...
  // Status Badge function (FIXED)
  const getBadgeVariant = (status: Member['status']) => {
    switch (status.toLowerCase()) {
      ...
      case "pending":
        return "outline";
      default:
        // Use approved as a fallback default if status somehow misses the union types
        return "default"; 
    }
  };
                        ...
                        <DropdownMenuContent align="end">
                          ...
                          {user?.role !== "panel" && (
                            <>
                              ...
                              {/* Using ternary to switch between valid status types */}
                              <DropdownMenuItem
